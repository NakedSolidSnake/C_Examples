...
{
	ptr = realloc(ptr, size); /* FIXME Choose another variable to attribute the result because can lose the previous allocation */
	if(ptr == NULL)
		abort();
	else
		return ptr;
...
{
	error(operation, strerror(errno));	//error message
}
