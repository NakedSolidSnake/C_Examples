...
bool cli_open (cli_t *cli, cli_option_t *option_list, uint16_t option_amount)
{
    bool status = false;
...
static void cli_print_menu (cli_t *cli)
{
    for (uint16_t i = 0; i < cli->option_amount; i++)
    {
        cli_option_t *op = &cli->option_list[i];
        fprintf(stdout, "%d. %s\n", op->index, op->text);
    }

    fprintf (stdout, "Doesn't use 0 as option. 0 is used to quit application.\n");
}
